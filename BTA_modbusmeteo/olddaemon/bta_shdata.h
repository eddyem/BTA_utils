...
#define  L_time  (sdt->l_time) /* время работы программы */
   unsigned int ppndd_a, ppndd_z, ppndd_p, ppndd_b;
...
#define  ppndd_B  (sdt->ppndd_b) /* код датчика ППНДД давления  */
   unsigned int dup_a, dup_z, dup_p, dup_f, dup_d;
...
#define  dup_D  (sdt->dup_d) /* код Грея датчика ДУП положения купола  */
   unsigned int low_a, low_z, low_p, low_f, low_d;
...
#define  low_D  (sdt->low_d) /* код отсчета положения купола  */
   unsigned int code_a, code_z, code_p, code_b, code_f, code_d;
...
#define Sys_Mesg(N) (sdt->sys_msg_buf[N])
  ...
  /* коды установки уровней доступа для клиентов */
   unsigned int code_lev1,code_lev2,code_lev3,code_lev4,code_lev5;
...
#define  code_Lev5  (sdt->code_lev5) /* "главный оператор" - все  операции */
  /* ограничение сетевого доступа */
   unsigned int netmask, netaddr, acsmask, acsaddr;
...
/* информация от PEP-контроллеров */
   unsigned int pep_code_a, pep_code_z, pep_code_p;
...
#define  PEP_code_P  (sdt->pep_code_p) /*   -  ""  -             P */
   unsigned int pep_sw_a, pep_sw_z, pep_sw_p;
...
#define  Sw_Sm_P   0x80                /* датчик дыма СПФ   */
   unsigned int pep_code_f, pep_code_d, pep_code_ri, pep_code_ro;
...
/* информация PEP-контроллера купола */
   unsigned int pep_code_di, pep_code_do;
...
static void bta_data_init() {
   ...
   if(sdt->magic != sdat.key.code  ||
      ...
      val_A=val_Z=val_B=val_D=val_T1=val_T2=val_T3=val_Wnd=val_Alp=val_Del=0.;
      val_P=200*3600.;
      val_F=100.;
...
static void get_shm_block( struct SHM_Block *sb, int server) {
   ...
   fprintf(stderr,"Create&attach shared memory segment '%s' %dbytes at %x \n",
	   sb->key.name, sb->size, (unsigned long int)sb->addr);
...
unsigned int cmd_src_ip=0;  /* IP-адр. источника для ОДНОЙ след.команды */
#else
extern int snd_id;
extern int cmd_src_pid;
extern unsigned int cmd_src_ip;
#endif
...
static void set_acckey(unsigned int newkey) {
   if (snd_id<0) return;
...
static void set_cmd_src(unsigned int ip, int pid) {
    cmd_src_pid = pid;
    cmd_src_ip = ip;
}
...
struct my_msgbuf {
   ...
   unsigned int acckey;  /* ключ доступа клиента */
   unsigned int src_pid; /* номер процесса источника */
   unsigned int src_ip;  /* IP-адр. источника, =0 - локальная команда */
...
static void encode_lev_passwd(char *passwd, int nlev, unsigned int *keylev, unsigned int *codlev) {
    char salt[4];
    char *encr;
    union {
      unsigned int ui;
...
static int find_lev_passwd(char *passwd, unsigned int *keylev, unsigned int *codlev) {
    int nlev;
